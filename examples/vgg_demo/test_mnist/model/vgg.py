...
Optimized with the same configuration-driven construction pattern as the
reference implementation in the project root model definition, while keeping
explicit activation and pooling modules for later FHE-oriented replacement.

For 32x32 CIFAR-10 input, feature map sizes after each pool layer:
  Block 1: 32 -> pool -> 16x16x64
  Block 2: 16 -> pool -> 8x8x128
  Block 3:  8 -> pool -> 4x4x256
  Block 4:  4 -> pool -> 2x2x512
  Block 5:  2 -> pool -> 1x1x512
...
class VGG(nn.Module):
    """Configuration-driven VGG adapted for CIFAR-10."""
...
def _vgg(model_name, in_channels=3, num_classes=10, batch_norm=True, init_weights=True):
...
def vgg11(in_channels=3, num_classes=10, init_weights=True):
...
def vgg19(in_channels=3, num_classes=10, init_weights=True):
    """Create VGG19 model for CIFAR-10."""
    return _vgg("vgg19", in_channels=in_channels, num_classes=num_classes, batch_norm=False, init_weights=init_weights)


def vgg19_bn(in_channels=3, num_classes=10, init_weights=True):
    """Create VGG19-BN model for CIFAR-10."""
