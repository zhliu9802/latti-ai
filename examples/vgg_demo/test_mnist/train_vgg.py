...
"""Train VGG16-BN on CIFAR-10 (baseline or poly-ReLU).

Baseline (standard ReLU):
  python train_vgg.py --epochs 200

Poly-ReLU (replace ReLU with RangeNormPoly2d, fine-tune, export):
  python train_vgg.py --poly_model_convert --pretrained train_baseline.pth --epochs 10 --input-shape 3 32 32
...
def main():
    parser = argparse.ArgumentParser(description='Train VGG on MNIST')
    ...
    parser.add_argument('--input-dir', default='./runs/vgg_demo/mnist/model')
    parser.add_argument('--output-dir', default='./runs/vgg_demo/mnist/model')
    parser.add_argument('--export-dir', default='./runs/mnist/task/server', help='directory for fused H5 (default: ./runs/mnist/task/server)')
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help='C H W input shape for ONNX export, e.g. --input-shape 1 32 32',
    )
    args = parser.parse_args()

    if args.poly_model_convert and args.input_shape is None:
        parser.error('--input-shape is required when --poly_model_convert is enabled, e.g. --input-shape 1 32 32')
    ...
    input_shape = args.input_shape or [1, 32, 32]
    train_loader, test_loader = get_mnist_loaders(args.data_dir, args.batch_size, args.num_workers, input_shape)

    # Build model
    # model = vgg16_bn()
    # model = vgg19_bn()
    model = vgg11_bn(num_classes=10, in_channels=input_shape[0])
    ...
    best_filename = 'train_poly.pth' if args.poly_model_convert else 'train_vgg_baseline.pth'
    best_acc = 0.0
